      ...
      function startListeners() {
        ...
        if (typeof listenBlog === "function" && !window.__blogListening) {
          listenBlog();
        }
        ...
        if (!window.__reviewsListening) {
          ...
          const q = query(
            collection(window.__db, "reviews"),
              ...
              grid.innerHTML = snap.docs
                ...
                .map((d) => {
                  ...
                  const initials = (r.nom || "?")
                    ...
                    .toUpperCase()
                    .slice(0, 2);
                  // Échappement HTML : r.txt, r.nom, r.loc viennent de visiteurs publics
                  return (
                    `<div class="review-card">` +
                    `<div class="rev-stars" style="color:var(--gold)">${stars}</div>` +
                    `<p class="rev-text">\u00ab\u00a0${escapeHTML(r.txt)}\u00a0\u00bb</p>` +
                    `<div class="rev-author"><div class="rev-avatar">${escapeHTML(initials)}</div>` +
                    `<div><div class="rev-name">${escapeHTML(r.nom)}</div><div class="rev-loc">${escapeHTML(r.loc || "Bamako, Mali")}</div></div></div>` +
                    `</div>`
                  );
                })
                .join("");
